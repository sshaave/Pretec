...
python3 makeVerElement.py $Bunn_b $Klakk_t $((Bak_h - sveis )) 0 $(( Opp_l )) $Bak_h $refP $move $Bunn_b_opp 0
python3 merge.py total multi

python3 makeVerElement.py $Bak_t $Klakk_t $(( Bak_h - sveis )) $Bunn_b $Opp_l $Bak_h $refP $move 0 0
python3 merge.py total multi
...
# -------   P1
# Stegplate, nr 15
python3 makeVerElement.py $Bak_t $Bak_l $(( Bak_h - sveis )) $Bunn_b $(( Opp_l + Klakk_t )) $(( Bak_h )) $refP $move 0 150
python3 merge.py total multi

python3 makeVerElement.py $Bak_t $Bak_l $sveis $Bunn_b $(( Opp_l + Klakk_t )) $(( sveis )) $refP $move 0 150
python3 merge.py total multi
